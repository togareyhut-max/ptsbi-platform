...
(function ($) {
    ...
    function syncRepeaterFields(form) {
        ...
        syncRepeaterType(form, 'wilayah', '#ptprm-wilayah-items-json', collectWilayahRow);
    }
    ...
    function getMenuSubmenuMax() {
        ...
        var n = rep ? parseInt(rep.getAttribute('data-submenu-max'), 10) : 15;
        return isNaN(n) ? 15 : Math.max(1, Math.min(15, n));
    }
    ...
    function collectWilayahRow(row) {
        var label = (row.querySelector('[data-field="label"]') || {}).value || '';
        label = label.trim();
        if (!label) return null;
        return {
    ...
    function repeaterMax(type) {
        ...
        if (type === 'team') return 5;
        if (type === 'wilayah') return 20;
        return 24;
    }
    ...
    function initRepeaters() {
        ...
        initRepeater('team', '#ptprm-team-repeater', '#ptprm-tpl-team-row', '#ptprm-team-items-json', function (row, data) {
            var imgEl = row.querySelector('[data-field="image"]');
            if (imgEl) imgEl.value = data.image || '';
            var groupEl = row.querySelector('[data-field="group"]');
        ...
        initRepeater('wilayah', '#ptprm-wilayah-repeater', '#ptprm-tpl-wilayah-row', '#ptprm-wilayah-items-json', function (row, data) {
            row.querySelector('[data-field="label"]').value = data.label || '';
            row.querySelector('[data-field="url"]').value = data.url || '';
        });
        ...
        document.querySelectorAll('.ptprm-repeater').forEach(function (wrap) {
            wrap.addEventListener('click', function (e) {
                if (e.target.classList.contains('ptprm-repeater-add')) {
                    ...
                    if (type === 'menu') {
                        ...
                        empty = { image: '', name: '', role: '', url: '', group: '' };
            ...
            wrap.addEventListener('click', function (e) {
                if (e.target.classList.contains('ptprm-repeater-remove')) {
                    ...
                    updateRepeaterAddState(wrap, list, repeaterMax(typeR));
                }
            });
            var list0 = wrap.querySelector('.ptprm-repeater-list');
            var type0 = wrap.getAttribute('data-type');
            updateRepeaterAddState(wrap, list0, repeaterMax(type0));
        });
    }
    ...
    function initRepeater(type, wrapSel, tplSel, hiddenSel, fillRow) {
        ...
        var max = type === 'menu' ? 12 : (type === 'team' ? 5 : (type === 'wilayah' ? 20 : 24));
        if (items.length > max) {
