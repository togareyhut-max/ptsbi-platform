...
class PTPRM_Address_Regions {
    ...
    public static function get_index(): array {
        ...
        $path = PTPRM_DIR . self::INDEX_FILE;
        if ( is_readable( $path ) ) {
    ...
    public static function get_province_tree( string $province_code ): array {
        ...
        $path = PTPRM_DIR . self::DATA_DIR . 'provinces/' . $province_code . '.json';
    ...
    public static function enqueue_scripts( string $handle, string $src, array $deps = [] ): void {
        wp_enqueue_script(
            ...
            PTPRM_URL . 'assets/js/address-regions.js',
            [],
            PTPRM_VERSION,
            true
        );
    ...
    public static function render_wilayah_select( string $current = 'jabodetabek' ): void {
        ...
        echo '<option value="jabodetabek"' . selected( $current, 'jabodetabek', false ) . '>' . esc_html__( 'Jabodetabek', 'ptsbi-premium' ) . '</option>';
        echo '<option value="all"' . selected( $current, 'all', false ) . '>' . esc_html__( 'Seluruh Indonesia', 'ptsbi-premium' ) . '</option>';
        echo '</select></label>';
        echo '<p class="ptprm-portal-help ptprm-member-full">' . esc_html__( 'Kemendagri: Provinsi → Kabupaten/Kota → Kecamatan → Kelurahan. Kode pos terisi otomatis jika tersedia.', 'ptsbi-premium' ) . '</p>';
    }
