...
class PTPRM_Admin_Portal {
    ...
    public function __construct() {
        ...
        add_action( 'init', [ $this, 'handle_approve_pending_registrations' ], 20 );
    ...
    public function handle_export(): void {
        ...
        if ( ! is_user_logged_in() || ! $this->can_access_members_tab() ) {
            wp_die( esc_html__( 'Akses ditolak.', 'ptsbi-premium' ) );
        }
        if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( (string) $_GET['_wpnonce'] ) ), 'ptprm_members_export_xlsx' ) ) {
    ...
    public function handle_approve_pending_registrations(): void {
        if ( empty( $_POST['ptprm_approve_member_id'] ) || ! $this->can_access_members_tab() ) {
            return;
        }
        ...
        $ok  = class_exists( 'PTPRM_Members' ) ? PTPRM_Members::approve_pending_registration( $user_id ) : false;
        $msg = $ok
            ? __( 'Pendaftaran berhasil di-approve.', 'ptsbi-premium' )
            : __( 'Gagal approve pendaftaran (mungkin sudah di-approve sebelumnya).', 'ptsbi-premium' );

        wp_safe_redirect( add_query_arg( 'ptprm_members_notice', rawurlencode( $msg ), self::portal_url( 'anggota' ) ) );
        exit;
    }
    ...
    public function shortcode_portal(): string {
        ...
        $tabs = [ 'ringkasan' => __( 'Ringkasan', 'ptsbi-premium' ) ];
    ...
    private function render_tab_members(): void {
        ...
        $export_url = wp_nonce_url(
            add_query_arg( 'ptprm_export_members', '1', self::portal_url( 'anggota' ) ),
            'ptprm_members_export_xlsx'
        );
        ...
        echo '<a class="ptprm-cta ptprm-cta-outline ptprm-cta-size-medium" href="' . esc_url( $export_url ) . '"><span class="ptprm-cta-label">' . esc_html__( 'Unduh XLSX', 'ptsbi-premium' ) . '</span></a>';
        echo '</div>';
    }
