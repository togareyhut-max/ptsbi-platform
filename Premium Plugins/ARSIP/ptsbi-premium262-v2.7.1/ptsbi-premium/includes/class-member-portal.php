...
class PTPRM_Member_Portal {
    ...
    private function render_profile_tab( ?array $record ): void {
        ...
        $text_fields = [
            ...
            'oppu'            => __( 'Ompu (Paroppuon)', 'ptsbi-premium' ),
        ...
        echo esc_html__( 'Status marga', 'ptsbi-premium' ) . ': <strong>' . esc_html( $status !== '' ? $status : '—' ) . '</strong>. ';
        echo esc_html__( 'Anak jika nama kepala ber-marga Samosir; Boru jika nama istri memakai Br. (boru).', 'ptsbi-premium' );
        echo '</p>';
        ...
        if ( class_exists( 'PTPRM_Address_Regions' ) ) {
            PTPRM_Address_Regions::render_wilayah_select( 'jabodetabek' );
        ...
        echo '<div class="ptprm-member-full ptprm-address-region-block" data-ptprm-address-intl style="display:none;">';
        echo '<label><span>' . esc_html__( 'State / Kota (luar negeri)', 'ptsbi-premium' ) . '</span>';
        echo '<input type="text" name="state_city" value="' . esc_attr( (string) ( $r['state_city'] ?? '' ) ) . '"></label>';
        echo '</div>';

        $addr = (string) ( $r['address_detail'] ?? '' );
        echo '<label class="ptprm-member-full"><span>' . esc_html__( 'Alamat lengkap (jalan, nomor rumah)', 'ptsbi-premium' ) . '</span>';
