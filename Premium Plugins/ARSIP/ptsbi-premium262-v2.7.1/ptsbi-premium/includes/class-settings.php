...
(function () {
    ...
    function syncRepeaters(form) {
        [
            ...
            { type: 'wilayah', hidden: '#ptprm-wilayah-items-json', wrap: '#ptprm-wilayah-repeater' }
            ...
            list.querySelectorAll('.ptprm-repeater-row').forEach(function (row) {
                if (cfg.type === 'values') {
                    ...
                    var tn = (row.querySelector('[data-field="name"]') || {}).value || '';
                    tn = tn.trim();
                    if (!tn) return;
                    items.push({
                ...
                } else if (cfg.type === 'wilayah') {
                    var wl = (row.querySelector('[data-field="label"]') || {}).value || '';
                    wl = wl.trim();
                    if (!wl) return;
                    items.push({
                        label: wl,
...
JS;
    ...
    public function sanitize( $input ) {
        ...
        $text_fields = [
            ...
            'values_eyebrow', 'values_title', 'values_hover_hint', 'team_layout', 'team_photo_style',
            'wilayah_title',
        ...
        if ( isset( $c['team_photo_style'] ) && ! in_array( $c['team_photo_style'], [ 'compact', 'card' ], true ) ) {
            $c['team_photo_style'] = 'compact';
        }
        ...
        $wilayah_raw   = $input['wilayah_items'] ?? ( $existing['wilayah_items'] ?? '' );
        $wilayah_clean = ptprm_sanitize_wilayah_items( $wilayah_raw );
        if ( ! $wilayah_clean ) {
            $wilayah_clean = ptprm_get_wilayah_items( $existing );
        }
        $c['wilayah_items'] = wp_json_encode( $wilayah_clean, JSON_UNESCAPED_UNICODE );

        $sub_max_raw = isset( $input['header_submenu_max'] )
            ? (int) $input['header_submenu_max']
            : (int) ( $existing['header_submenu_max'] ?? $d['header_submenu_max'] ?? 15 );
    ...
    private function panel_header( $o, $opt ) {
        ...
        <section class="ptprm-panel" id="tab-header" data-panel="header" hidden>
            ...
            <div class="ptprm-card">
                ...
                $this->number( __( 'Maks. item submenu per menu utama', 'ptsbi-premium' ), 'header_submenu_max', $o, $opt, 1, 15 );
    ...
    private function panel_beranda( $o, $opt ) {
        ...
        <section class="ptprm-panel" id="tab-beranda" data-panel="beranda" hidden>
            ...
            <div class="ptprm-card">
                ...
                <p class="ptprm-help"><?php esc_html_e( 'Form pendaftaran publik dan direktori pencarian (login). Data disimpan di tabel database, bukan di pengaturan plugin.', 'ptsbi-premium' ); ?></p>
                <?php
                $this->bool( __( 'Tampilkan form daftar anggota', 'ptsbi-premium' ), 'members_register_show', $o, $opt );
                $this->bool( __( 'Tampilkan direktori cari anggota', 'ptsbi-premium' ), 'members_directory_show', $o, $opt );
            ...
            <div class="ptprm-card">
                ...
                <div class="ptprm-grid-2">
                    ...
                    <div>
                        ...
                        $this->select( 'Gaya foto', 'team_photo_style', $o, $opt, [
                            'compact' => 'Kompak — foto kecil (maks. 5 pengurus)',
                            'card'    => 'Kartu besar (maks. 5 pengurus)',
                        ] );
                        $this->number( 'Kolom maks. desktop (2–5)', 'team_columns', $o, $opt, 2, 5 );
                ...
                <div class="ptprm-mini-card" style="margin-top:1.5rem;">
                    <h4><?php esc_html_e( 'Box Pengurus Wilayah', 'ptsbi-premium' ); ?></h4>
                    <p class="ptprm-help"><?php esc_html_e( 'Tampil di bawah foto pengurus inti. Setiap item mengarah ke halaman pengurus wilayah (isi halaman dengan shortcode [ptprm_wilayah_board]).', 'ptsbi-premium' ); ?></p>
                    <?php $this->bool( 'Tampilkan box wilayah', 'wilayah_show', $o, $opt ); ?>
                    <?php $this->t( 'Judul box', 'wilayah_title', $o, $opt ); ?>
                    <?php $this->ta( 'Keterangan singkat', 'wilayah_intro', $o, $opt, 2 ); ?>
                    <?php $this->repeater_wilayah( $o, $opt ); ?>
                </div>
            </div>
    ...
    private function repeater_header_menu( $o, $opt ) {
        ...
        <div class="ptprm-repeater" id="ptprm-menu-repeater" data-type="menu" data-tpl="#ptprm-tpl-menu-row" data-submenu-max="<?php echo esc_attr( (string) $sub_max ); ?>">
            <p class="ptprm-help"><?php esc_html_e( 'Menu dropdown kustom: item utama = label menu atas. Klik “+ Tambah submenu” untuk daftar turunan (mis. Pengurus Wilayah → Jabodetabek, Medan). Alternatif: pakai Menu WordPress dengan item bersarang.', 'ptsbi-premium' ); ?></p>
    ...
    private function repeater_team( $o, $opt ) {
        ...
        <div class="ptprm-repeater" id="ptprm-team-repeater" data-type="team" data-tpl="#ptprm-tpl-team-row">
            <p class="ptprm-help">Maksimal <strong>5 pengurus</strong> dengan foto kecil. Isi <strong>Grup</strong> bila memakai tata letak kelompok.</p>
            <div class="ptprm-repeater-list"></div>
    ...
    private function repeater_wilayah( $o, $opt ) {
        $items    = ptprm_get_wilayah_items( $o );
        $json     = wp_json_encode( $items, JSON_UNESCAPED_UNICODE );
        $json_b64 = base64_encode( (string) $json );
        ?>
        <input type="hidden" id="ptprm-wilayah-items-json" value="" data-ptprm="wilayah_items" data-ptprm-json-b64="<?php echo esc_attr( $json_b64 ); ?>" />
        <div class="ptprm-repeater" id="ptprm-wilayah-repeater" data-type="wilayah" data-tpl="#ptprm-tpl-wilayah-row">
            <div class="ptprm-repeater-list"></div>
            <p><button type="button" class="button button-secondary ptprm-repeater-add">+ <?php esc_html_e( 'Tambah wilayah', 'ptsbi-premium' ); ?></button></p>
        </div>
        <script type="text/template" id="ptprm-tpl-wilayah-row">
            <div class="ptprm-repeater-row ptprm-mini-card" data-index="{{index}}">
                <div class="ptprm-repeater-row-head">
                    <strong><?php esc_html_e( 'Wilayah', 'ptsbi-premium' ); ?> {{num}}</strong>
                    <button type="button" class="button-link-delete ptprm-repeater-remove"><?php esc_html_e( 'Hapus', 'ptsbi-premium' ); ?></button>
                </div>
                <label class="ptprm-field"><span class="ptprm-label"><?php esc_html_e( 'Nama wilayah', 'ptsbi-premium' ); ?></span>
                    <input type="text" class="regular-text" data-field="label" value="" placeholder="mis. Jabodetabek" /></label>
                <label class="ptprm-field"><span class="ptprm-label">URL halaman</span>
                    <input type="text" class="regular-text" data-field="url" value="" placeholder="/pengurus-jabodetabek/" /></label>
            </div>
        </script>
        <?php
    }
