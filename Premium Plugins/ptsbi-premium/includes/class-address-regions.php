...
class PTPRM_Address_Regions {
    ...
    public static function enqueue_scripts( string $handle, string $src, array $deps = [] ): void {
        ...
        wp_enqueue_script( $handle, $src, $all_deps, PTPRM_VERSION, true );
    }
    ...
    public static function render_wilayah_select( string $current = 'jabodetabek' ): void {
        ...
        echo '<span>' . esc_html__( 'Wilayah', 'ptsbi-premium' ) . '</span>';
        echo '<select name="address_wilayah" data-ptprm-wilayah data-current="' . esc_attr( $current ) . '">';
        echo '<option value="jabodetabek"' . selected( $current, 'jabodetabek', false ) . '>' . esc_html__( 'Jabodetabek (Jakarta, Bogor, Tangerang, Bekasi)', 'ptsbi-premium' ) . '</option>';
        echo '<option value="all"' . selected( $current, 'all', false ) . '>' . esc_html__( 'Seluruh Indonesia', 'ptsbi-premium' ) . '</option>';
        echo '<option value="overseas"' . selected( $current, 'overseas', false ) . '>' . esc_html__( 'Luar Negeri', 'ptsbi-premium' ) . '</option>';
        echo '</select></label>';
        echo '<p class="ptprm-portal-help ptprm-member-full">' . esc_html__( 'Jabodetabek: Jakarta & penyangga. Seluruh Indonesia: semua provinsi. Luar negeri: isi alamat manual.', 'ptsbi-premium' ) . '</p>';
    }
