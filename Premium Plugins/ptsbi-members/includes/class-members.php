...
if ( class_exists( 'PTPRM_Members', false ) ) {
    return;
}
...
class PTPRM_Members {
    ...
    public static function render_status_select( string $current, string $kepala = '', string $istri = '' ): void {
        ...
        echo '<label><span>' . esc_html__( 'Status', 'ptsbi-members' ) . '</span>';
        ...
        if ( $auto !== '' && $auto !== $current ) {
            echo '<p class="ptprm-portal-help ptprm-member-full">' . esc_html__( 'Saran otomatis dari marga', 'ptsbi-members' ) . ': <strong>' . esc_html( $auto ) . '</strong></p>';
        }
    }
        ...
        while ( true ) {
            ...
            foreach ( $chunk as $r ) {
                $dataset[] = [
                    ...
                    (string) $r['state_city'],
                    self::compute_marga_status( (string) $r['kepala_keluarga'], (string) $r['nama_istri'], (string) $r['hula_boru'] ),
                    (string) $r['phone'],
                ];
            }
    ...
    private function render_directory_filters( array $filters ): void {
        ...
        echo '<label><span>' . esc_html__( 'Wilayah', 'ptsbi-members' ) . '</span>';
        echo '<select name="wilayah" data-ptprm-wilayah data-current="jabodetabek">';
        echo '<option value="jabodetabek"' . selected( (string) ( $filters['wilayah'] ?? 'jabodetabek' ), 'jabodetabek', false ) . '>' . esc_html__( 'Jabodetabek', 'ptsbi-members' ) . '</option>';
        echo '<option value="all"' . selected( (string) ( $filters['wilayah'] ?? '' ), 'all', false ) . '>' . esc_html__( 'Seluruh Indonesia', 'ptsbi-members' ) . '</option>';
        echo '</select></label>';
