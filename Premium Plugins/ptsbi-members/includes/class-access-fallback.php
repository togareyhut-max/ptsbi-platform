...
class PTPRM_Access {
    ...
    public static function portal_url_for_user( $user = null ): string {
        ...
        if ( class_exists( 'PTPRM_Bidang_Portal' ) && class_exists( 'PTPRM_Bidang_Registry' ) ) {
            $bidang = PTPRM_Bidang_Registry::user_bidang_slug( $user );
            if ( $bidang !== '' && PTPRM_Bidang_Registry::get( $bidang ) && self::is_manager( $user ) ) {
                return PTPRM_Bidang_Registry::panel_url( $bidang );
            }
        }
