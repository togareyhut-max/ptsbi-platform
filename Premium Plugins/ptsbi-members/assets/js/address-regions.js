...
(function (global) {
    ...
    function initCascade(root, options) {
        ...
        function currentWilayah() {
            return wilayahSel ? (wilayahSel.value || 'jabodetabek') : (options.defaultWilayah || 'jabodetabek');
        }

        function toggleOverseas() {
            var on = currentWilayah() === 'overseas';
            if (overseas && overseas.type === 'checkbox') {
                on = overseas.checked;
            }
            if (idBlock) idBlock.style.display = on ? 'none' : '';
            if (intlBlock) intlBlock.style.display = on ? '' : 'none';
            var flag = root.querySelector('[data-ptprm-overseas-flag]');
        ...
        function restoreSaved() {
            if (wilayahSel && saved.wilayah) wilayahSel.value = saved.wilayah;
            var provinces = filterProvinces(currentWilayah());
