...
class PTPRM_Members {
    ...
    public static function export_download_url(): string {
        $base = class_exists( 'PTPRM_Admin_Portal' )
            ...
            : ( class_exists( 'PTPRM_Admin_Portal' ) ? PTPRM_Admin_Portal::portal_url() : home_url( '/panel-admin/' ) );
    ...
    public function shortcode_registration(): string {
        ...
        echo '<label><span>' . esc_html__( 'Nama', 'ptsbi-premium' ) . ' *</span><input type="text" name="name" id="ptprm-reg-name" autocomplete="off" required></label>';
        echo '<label><span>' . esc_html__( 'Email (sebagai username)', 'ptsbi-premium' ) . ' *</span><input type="email" name="email" id="ptprm-reg-email" autocomplete="off" autocapitalize="off" required></label>';
        echo '<label><span>' . esc_html__( 'No HP', 'ptsbi-premium' ) . ' *</span><input type="tel" name="phone" id="ptprm-reg-phone" autocomplete="off" required></label>';
        echo '<label><span>' . esc_html__( 'Password', 'ptsbi-premium' ) . ' *</span><input type="password" name="password" id="ptprm-reg-pass" minlength="6" autocomplete="new-password" required></label>';
        echo '</div>';
        ...
        if ( function_exists( 'ptprm_form_autofill_guard_script' ) ) {
            ptprm_form_autofill_guard_script( 'ptprm-reg-email', 'ptprm-reg-pass' );
        }
        if ( class_exists( 'PTPRM_Member_Portal' ) ) {
    ...
    private function redirect_admin_notice( string $message ): void {
        $base = class_exists( 'PTPRM_Admin_Portal' )
            ? PTPRM_Admin_Portal::portal_url( 'anggota' )
            : ( class_exists( 'PTPRM_Admin_Portal' ) ? PTPRM_Admin_Portal::portal_url( 'anggota' ) : home_url( '/panel-admin/?tab=anggota' ) );
