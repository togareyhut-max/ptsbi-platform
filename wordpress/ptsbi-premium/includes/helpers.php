...
/**
 * Form pendaftaran anggota publik (beranda & halaman login). Default: nonaktif.
 */
...
/**
 * Matikan form login/pendaftaran publik di situs yang sudah berjalan (sekali).
 */
...
function ptprm_normalize_option_for_storage( array $o ): array {
    ...
    // Pengurus dihapus dari UI — jangan migrasi berat setiap request (hemat memori).
 ...
 */
function ptprm_parse_menu_submenu_lines( string $text, int $max = 10 ): array {
...
function ptprm_header_submenu_max( ?array $o = null ): int {
    if ( is_array( $o ) && array_key_exists( 'header_submenu_max', $o ) ) {
        return max( 1, min( 20, (int) $o['header_submenu_max'] ) );
    }
    return 10;
}
...
/**
 * Cek apakah tabel anggota sudah dibuat oleh migrasi plugin.
 */
function ptprm_members_table_exists(): bool {
    global $wpdb;
