...
class PTPRM_Member_Portal {
    ...
    public function handle_profile_save(): void {
        ...
        if ( ! self::is_anggota() && ! current_user_can( 'manage_options' ) && ! PTPRM_Members::is_pending_registration_user( $user_id ) ) {
            wp_safe_redirect( add_query_arg( 'ptprm_profile_error', rawurlencode( __( 'Akun ini tidak dapat mengubah profil anggota.', 'ptsbi-premium' ) ), self::navigation_url( [ 'tab' => 'profil' ] ) ) );
            exit;
        }
        ...
        if ( empty( $_POST['ptprm_password_only'] ) ) {
            ...
            if ( ! $saved ) {
                ...
                $detail = ( defined( 'WP_DEBUG' ) && WP_DEBUG && ! empty( $wpdb->last_error ) )
                    ? ' (' . $wpdb->last_error . ')'
                    : '';
                wp_safe_redirect( add_query_arg( 'ptprm_profile_error', rawurlencode( __( 'Profil tidak tersimpan. Periksa data lalu coba lagi.', 'ptsbi-premium' ) . $detail ), self::navigation_url( [ 'tab' => 'profil' ] ) ) );
                exit;
            }
        }
    ...
    private function render_profile_tab( ?array $record ): void {
        ...
        if ( $status !== '' ) {
            ...
            echo esc_html__( 'Status marga (otomatis jika kosong)', 'ptsbi-premium' ) . ': <strong>' . esc_html( $status ) . '</strong>';
            echo '</p>';
        }
        ...
        if ( class_exists( 'PTPRM_Address_Regions' ) ) {
            ...
            PTPRM_Address_Regions::render_select( 'province', __( 'Provinsi', 'ptsbi-premium' ), $province, [ 'data-current' => $province ] );
            PTPRM_Address_Regions::render_select( 'city', __( 'Kabupaten / Kota', 'ptsbi-premium' ), $city, [ 'data-current' => $city ] );
            PTPRM_Address_Regions::render_select( 'district', __( 'Kecamatan', 'ptsbi-premium' ), (string) ( $r['district'] ?? '' ), [ 'data-current' => (string) ( $r['district'] ?? '' ) ] );
        ...
        echo '<div class="ptprm-member-grid">';
        echo '<label><span>' . esc_html__( 'Negara (luar negeri)', 'ptsbi-premium' ) . '</span>';
        echo '<input type="text" name="country_name_intl" value="' . esc_attr( $overseas ? $country : '' ) . '" data-ptprm-country-intl></label>';
        echo '<label><span>' . esc_html__( 'State / Kota', 'ptsbi-premium' ) . '</span>';
        echo '<input type="text" name="state_city" value="' . esc_attr( (string) ( $r['state_city'] ?? '' ) ) . '"></label>';
        echo '<label class="ptprm-member-full"><span>' . esc_html__( 'Alamat lengkap (manual)', 'ptsbi-premium' ) . '</span>';
